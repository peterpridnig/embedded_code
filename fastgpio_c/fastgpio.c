...
static dev_t fastgpio_dev_number = MKDEV(510,0);
...
static int driver_open( struct inode *geraete_datei, struct file *instanz )
{
	...
	printk("driver_open\n");
	err = gpio_request( 122, "rpi-gpio-122" );
	if (err) {
		printk("gpio_request failed %d\n", err);
		return -1;
	}
	err = gpio_direction_output( 122, 0 );
	if (err) {
		printk("gpio_direction_output failed %d\n", err);
		gpio_free( 122 );
		return -1;
	}
	printk("gpio 122 successfull configured\n");
	return 0;
}
...
static int __init mod_init( void )
{
	if( register_chrdev_region(fastgpio_dev_number,1,"fastgpio")<0 ) {
		printk("devicenumber (510,0) in use!\n");
		return -EIO;
	}

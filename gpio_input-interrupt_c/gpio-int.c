 ...
 * between P8 18 ground.
 *
 * gpio8_18 is configured as gpio 33, so to make it an input which
 * triggers on a falling edge, write
 *
 * echo 33 > /sys/class/gpio/export
 * echo falling > /sys/class/gpio/gpio33/edge
 *
 * Now, the gpio8_18 pin is normally pulled high, so
 * /sys/class/gpio33/value reads as 1'.
...
int main(int argc, char *argv[])
{
	...
	f = open("/sys/class/gpio/gpio33/value", O_RDONLY | O_NONBLOCK);
	if (f == -1) {
		perror("Can't open gpio33");
		return 1;
	}
